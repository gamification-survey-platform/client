...
const TriviaPopup = ({ courseId, courses }) => {
  ...
  const showNextHint = () =>
    setCurrentHintIndex((prevIndex) =>
      Math.min(prevIndex + 1, trivias[currentTriviaIndex]?.hints.length - 1)
    )
  ...
  const handleAnswerSubmit = async () => {
    try {
      const response = await markTriviaAsCompleted(
        trivias[currentTriviaIndex].id,
        hintsUsed,
        userAnswer
      )
      if (response.is_correct) {
        messageApi.open({
          type: 'success',
          content: `Correct answer! 🎉 You gained ${response.points} points.`
        })
      } else {
      ...
      if (updatedTrivias.length > 0) {
        const newIndex =
          currentTriviaIndex >= updatedTrivias.length
            ? updatedTrivias.length - 1
            : currentTriviaIndex
      ...
      messageApi.open({
        type: 'error',
        content: 'Failed to record trivia attempt. Please try again later.'
      })
    }
    setUserAnswer('')
  }
    ...
    <>
      ...
      {allCompleted && (
        <Modal
          ...
          title={
            <div style={{ fontWeight: 'bold', fontSize: '24px', color: '#3e79f7' }}>
              Trivia Completed!
            </div>
          }
          width={800}
          ...
          <p style={{ fontSize: '22px' }}>
            🎆🎆🎉🎉You have completed all trivia for this course. Look at other courses for more
            trivia fun! 🎓🚀
          </p>
        </Modal>
      )}
        ...
        <Modal
            ...
            trivias.length > 0
              ? [
                  ...
                  <Button
                    key="next"
                    onClick={nextTrivia}
                    disabled={currentTriviaIndex === trivias.length - 1}>
                    Next Trivia
                  </Button>,
                  <Button key="submit" type="primary" onClick={handleAnswerSubmit}>
                    Submit and earn
                    <Badge
                      count={calculatePoints()}
                      style={{ backgroundColor: '#00A86B', marginLeft: 4, marginRight: 4 }}
                    />
                    points
                  </Button>
                ]
          ...
          {trivias.length > 0 ? (
            <div
              ...
              {currentHintIndex === -1 && trivias[currentTriviaIndex].hints.length > 0 && (
                <Card
                  ...
                  <Button type="primary" onClick={useHint} icon={<QuestionCircleOutlined />}>
                    Use Hint (
                    <Badge
                      count={`-${6}`}
                      style={{ backgroundColor: '#f5222d', marginLeft: 4, marginRight: 4 }}
                    />
                    Points)
                  </Button>
                </Card>
                  ...
                  extra={
                    <>
                      ...
                      <Button
                        ...
                        Next Hint (
                        <Badge
                          count={`-${Math.ceil(calculatePoints() / 2)}`}
                          style={{ backgroundColor: '#f5222d', marginLeft: 4, marginRight: 4 }}
                        />
                        points)
                      </Button>
            ...
            <p
              ...
              😅 There is no trivia available for you! Please explore other courses for more trivia
              challenges and fun! 🎓🚀
            </p>
          )}
        </Modal>
